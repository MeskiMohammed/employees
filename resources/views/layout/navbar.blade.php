            ...
            <div class="flex items-center space-x-4">
                ...
                <div class="dropdown dropdown-end">
                    <button class="flex items-center space-x-3 focus:outline-none">
                        <img class="h-8 w-8 rounded-full border border-base-200" src="{{ Auth::user()->employee ? asset('storage/'.Auth::user()->employee->profile_picture) : 'https://img.freepik.com/premium-vector/character-avatar-isolated_729149-194801.jpg?semt=ais_hybrid&w=740' }}" alt="User avatar">
                        <div class="hidden md:block text-left">
                            <h6 class="text-sm font-medium">{{ Auth::user()->first_name." ".Auth::user()->last_name }}</h6>
                            <p class="text-xs text-base-content/60">{{ Auth::user()->hasRole('super_admin')?'super admin':Auth::user()->employee->typeEmployees->last()->type->type }}</p>
                        </div>
                    ...
                    <div class="dropdown-content bg-base-100 rounded-box shadow-lg mt-2 w-48 z-[50]">
                        <ul class="menu menu-sm w-full">
                            <li>
                                <a href="{{ Auth::user()->hasRole('super_admin') ? route('profile.edit') : route('employee.dashboard') }}" class="flex items-center px-4 py-2">

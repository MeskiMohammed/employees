    ...
    <div class="bg-base-200 border-base-300 shadow rounded-lg">
        ...
        <form action="{{ route('employees.update', $employee) }}" method="POST" enctype="multipart/form-data" class="p-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6" x-data="{freelancer : '{{ $selectedType }}' }">
                ...
                <div x-show="freelancer==='freelancer'" class="col-span-3 grid grid-cols-1 md:grid-cols-3 gap-6" x-data="{salary:{{ old('is_project', $employee->is_project) ? 'false' : 'true' }}}">
                    ...
                    <div x-show="salary">
                        ...
                        <input type="number" name="hourly_salary" id="hourly_salary" min="0" value="{{ old('hourly_salary', $employee->hourly_salary) }}" class="bg-base-100 shadow-sm focus:ring-indigo-500 focus:border-indigo-500 block w-full sm:text-sm border-base-300 rounded-md @error('hourly_salary') border-red-500 @enderror">
                        @error('hourly_salary')
                        <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
                        @enderror
                    </div>
            ...
            <div class="mt-6 flex items-center justify-end">
                <a href="{{ route('employees.index') }}" class="px-4 py-2 bg-gray-200 text-gray-700 rounded-md hover:bg-gray-300 button-white mr-4">
                    Cancel
                </a>

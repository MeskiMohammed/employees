...
<div class="max-w-4xl mx-auto">
    <div class="bg-white shadow rounded-lg">
        ...
        <form action="{{ route('employee-posts.store') }}" method="POST" class="p-6">
            @csrf
            
            <div class="mb-6">
                <label for="post" class="block text-sm font-medium text-gray-700 mb-1">Post Name</label>
                <input type="text" name="post" id="post" value="{{ old('post') }}" class="shadow-sm focus:ring-indigo-500 focus:border-indigo-500 block w-full sm:text-sm border-gray-300 rounded-md @error('post') border-red-500 @enderror" required>
                @error('post')

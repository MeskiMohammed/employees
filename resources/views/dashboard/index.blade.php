                                    ...
                                    <div class="flex items-center">
                                        <div class="flex-shrink-0 h-10 w-10">
                                            @if($employee->profile_picture)
                                                <img class="h-10 w-10 rounded-full" src="{{ asset('storage/' . $employee->profile_picture) }}" alt="{{ $employee->first_name }} {{ $employee->last_name }}">
                                            @else
                                                <div class="h-10 w-10 rounded-full bg-indigo-600 flex items-center justify-center">
                                                    <span class="text-xs font-medium text-white">{{ substr($employee->first_name, 0, 1) }}{{ substr($employee->last_name, 0, 1) }}</span>
                                                </div>
                                            @endif
                                        </div>
                                        <div class="ml-4">
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        Object.entries(monthlyData).forEach(([month, value]) => {
            monthlyValues[parseInt(month) - 1] = value;
        });

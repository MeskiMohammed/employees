...
<div class="bg-white shadow rounded-lg">
    <div class="flex justify-between items-center p-6 border-b">
        <h2 class="text-xl font-semibold text-gray-800">Statuses List</h2>
    ...
    <div class="p-6 border-b">
        <form action="{{ route('statuses.index') }}" method="GET" class="flex gap-4">
            <div class="flex-1">
                ...
                <input type="text" name="search" id="search" value="{{ request('search') }}" class="shadow-sm focus:ring-indigo-500 focus:border-indigo-500 block w-full sm:text-sm border-gray-300 rounded-md" placeholder="Search statuses...">
            </div>
            ...
            <tbody class="bg-white divide-y divide-gray-200">
                ...
                <tr>
                    <td class="px-6 py-4 whitespace-nowrap">
                        <div class="text-sm text-gray-900">{{ $status->id }}</div>
                    </td>
                    <td class="px-6 py-4 whitespace-nowrap">
                    ...
                    <td class="px-6 py-4 whitespace-nowrap">
                        <div class="text-sm text-gray-900">{{ $status->created_at->format('M d, Y') }}</div>
                        <div class="text-sm text-gray-500">{{ $status->created_at->format('h:i A') }}</div>
                    </td>

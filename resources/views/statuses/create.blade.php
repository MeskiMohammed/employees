...
<div class="max-w-4xl mx-auto">
    <div class="bg-white shadow rounded-lg">
        <div class="p-6 border-b">
            <h2 class="text-xl font-semibold text-gray-800">Create New Status</h2>
        </div>
        ...
        <form action="{{ route('statuses.store') }}" method="POST" class="p-6">
            ...
            <div class="mb-6">
                ...
                <input type="text" name="status" id="status" value="{{ old('status') }}" class="shadow-sm focus:ring-indigo-500 focus:border-indigo-500 block w-full sm:text-sm border-gray-300 rounded-md @error('status') border-red-500 @enderror" required>
                @error('status')

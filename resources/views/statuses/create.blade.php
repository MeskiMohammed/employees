    ...
    <div class="bg-base-200 shadow rounded-lg">
        ...
        <form action="{{ route('statuses.store') }}" method="POST" class="p-6">
            ...
            <div class="mb-6">
                ...
                <input type="text" name="status" id="status" value="{{ old('status') }}" class="bg-base-100 shadow-sm focus:ring-indigo-500 focus:border-indigo-500 block w-full sm:text-sm border-gray-300 rounded-md @error('status') border-red-500 @enderror" required>
                @error('status')
